...
export const sendArchivedData = (plan) => {
    return async () => {
        const sendRequest = async () => {
            const response = await fetch(
                "https://myprojectname-8bdfe-default-rtdb.firebaseio.com/archive.json",
                { method: "PUT", body: JSON.stringify(plan) }
            );
            if (!response.ok) {
                throw new Error("Sending cart data failed");
...
export const fetchArchivedData = () => {
    return async (dispatch) => {
        const fetchData = async () => {
            const response = await fetch(
                "https://myprojectname-8bdfe-default-rtdb.firebaseio.com/archive.json"
        ...
        try {
            ...
            dispatch(
                planActions.replaceArchivedInfo({
                    archived: allPlansData || [],
                })
            );
        } catch (error) {
            console.log(error.message);
